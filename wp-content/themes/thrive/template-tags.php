...
if ( ! function_exists( 'thrive_posted_on_single_new' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */

function thrive_posted_on_single_new() {
	$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}
	...
	$posted_on_single = sprintf(
		esc_html_x( '%s', 'post date', 'thrive' ), $time_string 
	);
        $byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'thrive' ),
		'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
	...
	echo '<span class="posted-on">' . $posted_on_single . '</span><span class="byline"> ' . $byline . '</span>'; // WPCS: XSS OK.
...
function thrive_posted_on_single() {
	$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}
	...
	echo '<span class="posted-on">' . $posted_on_single . '</span>'; // WPCS: XSS OK.
}
endif;
...
function thrive_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && thrive_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( '%1$s', 'thrive' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'thrive' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'thrive' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'thrive' ), esc_html__( '1 Comment', 'thrive' ), esc_html__( '% Comments', 'thrive' ) );
		echo '</span>';
	}
